...
import { Patient, ModalType, Demographics } from '../types';
...
const Patients: React.FC<PatientsProps> = ({ openModal, viewPatientDetails }) => {
  ...
  const renderContent = () => {
      ...
      if (filteredPatients.length === 0 && searchTerm !== '') {
          return (
             <EmptyState 
                message={`No patients found for "${searchTerm}".`}
                icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>}
            />
          )
      }
              ...
              return (
                <tr key={patient.id} className="border-b border-gray-200 hover:bg-gray-50/50 transition-colors">
                  <td className="p-4 flex items-center">
                    <img src={patient.avatar_url ?? ''} alt={patient.name} className="w-10 h-10 rounded-full mr-4" />
                    <div>
                      <p className="font-semibold text-gray-800">{patient.name}</p>
                      <p className="text-sm text-gray-500">{demographics?.gender}</p>
                    </div>
                  </td>
                  <td className="p-4 text-gray-700">{demographics?.contact}</td>
                  <td className="p-4 text-gray-700 hidden md:table-cell">{demographics?.dob}</td>
                  <td className="p-4 text-gray-700 hidden lg:table-cell">{patient.last_visit}</td>
