...
const getStatusColor = (status: ClaimStatus) => {
    switch (status) {
        ...
        case ClaimStatus.Denied: return 'bg-red-100 text-red-800';
    }
};
        ...
        return (
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {billing.map(claim => (
                            <tr key={claim.claimId} className="border-b border-gray-200 last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                <td className="p-4 font-mono text-sm text-gray-700">{claim.claimId}</td>
                                <td className="p-4 text-gray-700">{claim.patientId}</td>
                                <td className="p-4 text-gray-700">{claim.dateOfService}</td>
                                <td className="p-4 text-gray-700">${claim.amount.toFixed(2)}</td>
                                <td className="p-4">
